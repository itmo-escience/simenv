...
import itmo.escience.simenv.environment.entities.{DataFile, Node}
...
class ScheduleTest {
  ...
  def testApplySchedule(): Unit = {
    var tasks: List[DaxTask] = List()
    tasks :+= new DaxTask("t_0", 30, List(new DataFile("in0", 50)), List(new DataFile("f0", 10)))
    tasks :+= new DaxTask("t_1", 10, List(new DataFile("in1", 30)), List(new DataFile("f1", 20)))
    tasks :+= new DaxTask("t_2", 15, List(new DataFile("in2", 40)), List(new DataFile("f2", 15)))
    var nodes: List[Node] = List()
  ...
  def testNodeExecuting(): Unit = {
    ...
    var task0: DaxTask = new DaxTask("t_0", 30, List(new DataFile("in0", 50)), List(file0))
    var task1: DaxTask = new DaxTask("t_1", 10, List(new DataFile("in1", 30)), List(file1))
