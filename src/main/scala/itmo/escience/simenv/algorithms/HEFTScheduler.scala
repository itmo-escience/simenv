...
import itmo.escience.simenv.environment.entitiesimpl.{MultiWfWorkload}
...
//    }
    ...
    val wf = context.workload.asInstanceOf[MultiWfWorkload[T]].apps.head
...
//    val nodes = context.environment.nodes.filter(x => x.status == Node.UP)
    ...
    val tasks = prioritize[T, N](wf.asInstanceOf[Workflow[T]], nodes, context)
 ...
 *
   ...
   * @return
   */
  private def prioritize[T <: Task, N <: Node](wf:Workflow[T], nodes:Seq[N], context:Context[T, N]): Seq[T] = {
