...
//import itmo.escience.simenv.environment.entitiesimpl.{BasicContext, CoreRamEnvironment, SingleAppWorkload}
...
//
//  def environmentToSolution(env: CoreRamEnvironment):EnvConfigurationSolution = {
//    val genes = env.vms.map(x => MappedVm(x.id, (x.cores, x.ram))).toList
//    new EnvConfigurationSolution(genes)
//  }
//
//  def solutionToEnvironment(solution: EnvConfigurationSolution, context: Context[DaxTask, CoreRamNode]): CoreRamEnvironment = {
//    // reconstruction of environment in context with new configuration of vms
//    val nodes = context.environment.nodes
//    val vms = context.environment.asInstanceOf[CoreRamEnvironment].vms
//    var newNodes: List[CoreRamNode] = List()
//    for (n <- nodes) {
...
//    val environment: CoreRamEnvironment = new CoreRamEnvironment(newNodes, networks)
//
...
//}
//
//class EnvConfigurationProblem(wf:Workflow, newSchedule:Schedule, nodes:Seq[CoreRamNode], context:Context[DaxTask, CoreRamNode]) extends Problem[EnvConfigurationSolution]{
//
...
//        env.asInstanceOf[CoreRamEnvironment].vmById(nodeId).asInstanceOf[CoreRamNode],
//        context)
//    }
//    //--
