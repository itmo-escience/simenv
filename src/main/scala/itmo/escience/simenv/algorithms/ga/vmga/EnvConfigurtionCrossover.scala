...
import itmo.escience.simenv.environment.entities.{PhysicalResource, CoreRamHddBasedNode, DaxTask, Context}
 ...
 */
class EnvConfigurtionCrossover(probability: Double, context:Context[DaxTask, CoreRamHddBasedNode]) extends CrossoverOperator[EnvConfigurationSolution]{
  ...
  private def doCrossover(p1: EnvConfigurationSolution, p2: EnvConfigurationSolution) ={
    ...
    if (nodes.size > 1) {
      ...
      val n1vms = n1.asInstanceOf[PhysicalResource].children.map(x => x.id)
      val n2vms = n2.asInstanceOf[PhysicalResource].children.map(x => x.id)
