...
class GAFixScheduler(crossoverProb:Double, mutationProb: Double, swapMutationProb: Double,
  ...
  override def schedule[T <: Task, N <: Node](context: Context[T, N], environment: Environment[N]): Schedule[T, N] = {
    val factory: CandidateFactory[WFSchedSolution] = new FixScheduleCandidateFactory[T, N](context, environment)

    val operators: util.List[EvolutionaryOperator[WFSchedSolution]] = new util.LinkedList[EvolutionaryOperator[WFSchedSolution]]()
    operators.add(new FixScheduleCrossoverOperator())
    operators.add(new FixScheduleMutationOperator[T, N](context, environment, mutationProb, swapMutationProb))
