 ...
 */
object EcgHEFTScheduler extends Scheduler{
...
//    val nodes = context.environment.nodes.filter(x => x.status == Node.UP)
    ...
    for (task <- tasks.filter(x => !fixed_tasks.contains(x.id))) {
      val item = nodes.map((node) => newSchedule.ecgFindTimeSlot(task, node, context, environment)).minBy(x => x.endTime)
      newSchedule.ecgPlaceTask(task, item.node, context, environment)
    }
    newSchedule
  }
